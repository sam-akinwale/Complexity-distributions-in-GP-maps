...
# Keep only values occurring MORE than this threshold
# ---------------------------------------------------
...
for path_str in files:
    ...
    # ---------------------------------------------------
    # Remove OPC values occurring <= min_count times
    # ---------------------------------------------------

    opc_counts = df["OPC"].value_counts()

    keep_opc = opc_counts[opc_counts > min_count].index
    ...
    # ---------------------------------------------------
    # Remove CUSPS values occurring <= min_count times
    # ---------------------------------------------------

    cusp_counts = df["CUSPS"].value_counts()

    keep_cusps = cusp_counts[cusp_counts > min_count].index
    ...
    print(f"Final rows after CUSPS filter  : {final_n}")
